...
//            auto methodName = QMetaEnum::fromType<Method>().valueToKey(d->method);
            r = manager()->request(request, QByteArray());
            qDebug() << "Data is not set for WebRequest but method is set to post. Empty "
                        "QByteArray will be used";
        }
        break;
    }

    if (d->response->selfProcess())
        d->response->processReply(r);
    else
...
bool WebRequest::alwaysEmitFinished() const
{
    return d->alwaysEmitFinished;
}
...
void WebRequest::setAlwaysEmitFinished(bool alwaysEmitFinished)
{
    if (d->alwaysEmitFinished == alwaysEmitFinished)
        return;

    d->alwaysEmitFinished = alwaysEmitFinished;
    emit alwaysEmitFinishedChanged(d->alwaysEmitFinished);
}
